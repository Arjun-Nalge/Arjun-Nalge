# Hi, I’m Arjun Nalge👋

I’m a learner transitioning into DevOps from a non-IT background (Law).  
I’m focused on building strong fundamentals in cloud computing, Linux, networking, and automation, and applying them through hands-on projects.

## 👨‍💻 What I’m learning
- Linux and system administration  
- AWS cloud services (EC2, VPC, IAM, Auto Scaling, Load Balancer)  
- Git and GitHub  
- Basic networking concepts  
- DevOps fundamentals and best practices  

## 🚀 What I’m working on
- Deploying applications using multi-tier architecture on AWS  
- Creating real-world DevOps practice projects  
- Improving scripting and automation skills  

## 🎯 My goal
To become a skilled DevOps engineer by combining structured learning with practical, real-world implementation.

## 📫 Let’s connect
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?logo=linkedin)](https://www.linkedin.com/in/arjun-nalge-313642398)

Feel free to explore my repositories and follow my learning journey.
